...
namespace ThrownTogether
{
    ...
    public static class KitchenStaffRoute
    {
        ...
        public static Vector3[] ToStation(Vector3 from,Transform target)
        {
            ...
            while(queue.Count>0)
            {
                ...
                if(delta.magnitude<=1.5f && Clear(p))
                {
                ...
                {
                    var next=cell+d;if(parents.ContainsKey(next)||next.x < -22 || next.x>22 || next.y < -13 || next.y>20)continue;
    ...
    // Washes the sink queue only. Never clears tables, cooks, or creates replacement plates.
    public sealed class KitchenDishwasher : MonoBehaviour
    {
        RestaurantDay day;WashingStation sink;SourceStation stock;DiningWalker walker;CarrySlot hands;bool returning;float retry;
        public void Initialize(RestaurantDay owner)
        {
            ...
            var grip=new GameObject("Clean plate");grip.transform.SetParent(go.transform,false);grip.transform.localPosition=new Vector3(0,1.25f,.65f);hands=grip.AddComponent<CarrySlot>();
        }
        bool Travel(Transform destination)
        {var route=KitchenStaffRoute.ToStation(walker.transform.position,destination);if(route==null){retry=1;return false;}walker.Go(route);return true;}
        public void Advance(float seconds)
        {
            ...
            if(hands.Item!=null)
            {
                if(!returning){if(Travel(stock.transform))returning=true;return;}
                if(!stock.ReturnCleanPlate(hands.Item))return;
                returning=false;Travel(sink.transform);return;
            }
            var delta=walker.transform.position-sink.transform.position;delta.y=0;
            if(delta.magnitude>1.6f){Travel(sink.transform);return;}
            walker.transform.LookAt(new Vector3(sink.transform.position.x,walker.transform.position.y,sink.transform.position.z));
            if(!sink.Busy){if(sink.TakeClean(hands) && Travel(stock.transform))returning=true;return;}
            sink.WashBy(this,seconds);
        }
