...
namespace ThrownTogether
{
    ...
    public static class ExpoOrderStrip
    {
        ...
        public static KitchenTicket[] Visible(RestaurantExpo expo)=>expo.OpenTickets.Take(6).ToArray();
        public static void Draw(RestaurantDay day,float opacity)
        {
            var expo=day.Expo;if(expo==null || day.Closed || day.AwaitingMenu)return;
            var tickets=Visible(expo);int slots=Mathf.Max(1,tickets.Length);
            ...
            string extra=expo.OpenTickets.Count>6?" • +"+(expo.OpenTickets.Count-6)+" orders":"";
            GUI.Label(new Rect(230,581,820,24),"EXPO • "+expo.ActiveCount+" / "+expo.Capacity+" making • Visit Expo: Fire / Hold • Serving always allowed"+extra,text);
            for(int i=0;i<slots;i++)
            {
                KitchenTicket ticket=i<tickets.Length?tickets[i]:null;
                ...
                if(ticket==null){GUI.Label(tile,"No orders",text);continue;}
