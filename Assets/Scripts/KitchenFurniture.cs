...
namespace ThrownTogether
{
    ...
    public sealed class KitchenFurniture : MonoBehaviour
    {
        public static readonly Vector3[] Slots={
            ...
            new Vector3(-.8f,0,-1.1f),new Vector3(-.8f,0,.55f),new Vector3(-.8f,0,2.2f),
            new Vector3(-2.75f,0,2.2f),new Vector3(-2.75f,0,.55f),new Vector3(-2.75f,0,-1.1f)
        };
        ...
        public void Initialize(KitchenLayout value)
        {
            ...
            foreach(int index in new[]{0,1,3,4,5,6,7,10})
        ...
        public void Register(string id,Transform root)
        {
            ...
            pieces.Add(new Piece{id=id,root=root,before=root.position,rotation=root.rotation,slot=Nearest(root.position)});
        }
        ...
        public bool Begin()
        {
            ...
            Editing=true;Held=-1;Selected=0;Message="Choose equipment, then a green slot. Occupied slots can swap.";return true;
        }
        ...
        public bool Save()
        {
            ...
            if(!Editing || !CanEdit || !Validate(out var problem)){Message="Layout is not available or blocked.";return false;}
            ...
            if(!RestaurantAccounts.Current.SetFurniture(SessionOptions.Kitchen,records)){Message=RestaurantAccounts.Current.Problem;return false;}
        ...
        {
            // Compare projected locations so D-pad directions match the actual 45-degree view.
            var camera=GetComponent<RestaurantHud>().gameplayCamera;var origin=(Vector2)camera.WorldToScreenPoint(Slots[Selected]);
            float best=float.MaxValue;int chosen=Selected;
            for(int i=0;i<Slots.Length;i++)
            {var delta=(Vector2)camera.WorldToScreenPoint(Slots[i])-origin;float along=Vector2.Dot(delta,direction.normalized);if(along<5)continue;float side=Mathf.Abs(delta.x*direction.y-delta.y*direction.x);float score=delta.magnitude+side*3;if(score<best){best=score;chosen=i;}}
            Selected=chosen;
        }
