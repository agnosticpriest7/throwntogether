...
namespace ThrownTogether.Tests
{
    public sealed class RecipeBookTests
    {
        ...
        {
            var book=Resources.Load<RecipeBook>("RecipeBook");Assert.That(book,Is.Not.Null);Assert.That(book.recipes.Length,Is.EqualTo(3));
        ...
        [Test] public void BookReflectsActualHotAndColdRecipeSteps()
        {
            ...
            foreach(var recipe in book.recipes)
            {
                var text=RecipeBook.Instructions(recipe);Assert.That(text,Does.Contain("1.5 seconds"));
