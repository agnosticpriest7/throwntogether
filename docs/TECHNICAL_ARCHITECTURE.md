...
### Compound cook (0.21.0)
CookProduction projects one physical allocation per plate or loose finished/in-process component across Expo tickets. Complete plates take priority before partial plates; no allocation grants authority to mutate food. KitchenCook revalidates shared CarrySlots, follows existing staff routes, uses normal timed processing, and assembles on ordinary counters. Full-counter recovery exchanges two occupied slots synchronously at physical reach; no food teleportation or new inventory schema. Player/employee cooperation remains through real objects, not forced recipe-specific ownership. Held tickets cannot start additional components; completed in-flight work is preserved. No new binding/control map or recipe data added.
