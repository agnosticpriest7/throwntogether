...
0.19.1 adds optional restock/bins fields to the existing per-kitchen assignment snapshot, with defensive copies and atomic saves. Restock jobs have one physical destination and do not use Expo deficits; one worker round-robins eligible bins. Existing fired-order ledger semantics remain unchanged. See DEVELOPMENT_NOTES.md for behavior and verification.
